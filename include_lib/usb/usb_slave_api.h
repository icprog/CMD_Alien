...
struct usb_io_t{
    const u8 *buf;
    u32 len:7;//max 64byte
    u32 ot:8;//unit is tick time
};
typedef s32 (*usb_io_cb_func)(u8 *buf,u32 len);
...
#define VENDOR_CLASS_ENABLE	0
u32 intr_ep_tx(const u8 *buf,u32 len,u32 ot);
u32 bulk_ep_tx(const u8 *buf,u32 len,u32 ot);
