...
int gap_set_ble_connection_interval(u16 conn_handle,u16 interval_min,u16 interval_max,u16 timeout);

extern int ble_user_send_test_key_num(u16 conn_handle,u8 key_num);
void ble_smart_init();
u8 get_smart_pass();
u32 ble_smart_user_data_send(u8 *data, u16 len);
u32  ble_speex_send(u8 *data,u16 len);
void ble_smart_link_addr_reg(const u8 addr[6]);
void ble_smart_link_key_reg(const u8 link_key[16]);
