...
#define BUILD_POINT_CNT               60 * 500L     // n * 2ms
#define CHARGE_MAX_CNT                80 * 60 * 2L  // n * 500ms
#define CHARGE_THRESHOLD_VALUE        0x7400L
...
static void charge_timer0_isr_callback()
{
    ...
    if ((charge_t0_cnt % 5) == 0) {
        ...
        if(IS_POWER_KEY_DOWN())
		{
			/* charge_putchar('k'); */
			key_cnt ++;
			if(key_cnt > 300)     //长按3s开机
				__this->quit_charge_flag = 1;
		}
		else
		{
			key_cnt = 0;
...
static void charge_deal(u16 vbat)
{
    ...
    case STEP_GET_LINE:
		...
		charge_printf("time:%d last:%x cur:%x D-value:%d vbat:%x min:%d\n",time_cnt,__this->last_bat_value,__this->point_average,value,vbat,min_value);

#endif
		if((__this->point_average - __this->last_bat_value <= BUILD_POINT_THRESHOLD_VALUE) && (__this->point_average > CHARGE_THRESHOLD_VALUE)){
            charge_full_deal();
        }
...
void charge_disconnect_bt_check(u8 val)
{
#if CHARGE_PROTECT_EN
    ...
    if (get_ldo5v_online_flag()) {                   //充电线插入
        ...
        if (!__this->send_msg_flag && send_msg_cnt > 10) {
			...
			#define SET_MINUTE_CNT(x)     x * 60 * 100L

			charge_start();

        	__this->charge_cnt  = 0;

			charge_printf("LDO IN : %d\n",val);

			if(val < 37)      //<3.7V
			{
        		__this->charge_max_cnt  = SET_MINUTE_CNT(75);
				charge_printf("75 mins\n");
			}
			else if(val < 40) //<4.0v 
			{
        		__this->charge_max_cnt  = SET_MINUTE_CNT(40);;
				charge_printf("40 mins\n");
			}
			else if(val < 41) //<4.1v
			{
        		__this->charge_max_cnt  = SET_MINUTE_CNT(20);;
				charge_printf("20 mins\n");
			}
			else
			{
        		__this->charge_max_cnt  = SET_MINUTE_CNT(5);;
				charge_printf("5 mins\n");
			}
#else
            JL_POWER->CON |= BIT(4);
#endif
...
#if KEEP_BT_CONNECT
			__this->charge_cnt ++;
			if(__this->charge_cnt >__this->charge_max_cnt && __this->charge_max_cnt)
				charge_full_deal();
#endif
		}
