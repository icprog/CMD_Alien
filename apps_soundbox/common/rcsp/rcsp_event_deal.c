...
u8 *rcsp_send_tmp_buf = NULL;
u8 *rcsp_send_BigData_buf = NULL;
...
typedef struct __RCSP_SMART_S
{
	u8 data[16];
	u8 data_len;
}RCSP_SMART_S;

static RCSP_SMART_S rcsp_smart_send;

/*----------------------------------------------------------------------------*/
/**@brief  bt smart rcsp 命令发送
   @param  cmd:私有命令，data：私有命令附带数据 len:附带命令长度, 最大值为15
   @return 
   @note 	注意这里的cmd必须是大于等于A0，并且未被使用的 
*/
/*----------------------------------------------------------------------------*/
tbool rcsp_smart_command_send(u8 cmd, u8 *data, u8 len)
{
	u32 timeout = 50;
	...
	if(len > 15)
	{
		printf("data len is over limit\n");
		return false;
	}

	if(cmd < CBW_CMD_SPEEX_SEND_START)
	{
		printf("an err command \n");
		return false;		
	}

	while(rcsp_com_report_flag & RCSP_REPORT_SMART_SEND_STATUS)
	{
		printf("W");
		if(timeout)
		{
		...
		{
			printf("cmd send timeout\n");
			break;	
		}
		os_time_dly(3);		
	}

	rcsp_smart_send.data_len = len;
	rcsp_smart_send.data[0] = cmd; 

	if((data != NULL) && (len != 0))
	{
		memcpy(&(rcsp_smart_send.data[1]), data, len);
	}
	rcsp_com_report_flag |= RCSP_REPORT_SMART_SEND_STATUS;

	return true;
}


static void Post_smart_command(void)
{
    u32 err = RCSP_OP_ERR_NONE;
	u8 *OperationData = rcsp_send_tmp_buf;/// + (sizeof(task_frame_parm_t)*3);
	...
	memset(OperationData,0x00,16);
	memcpy(OperationData, rcsp_smart_send.data, rcsp_smart_send.data_len + 1);
...
u8 rcsp_event_cbk_send_process(int send_allow_flag)
{
	...
	/* putchar('B'); */
		...
		if(rcsp_com_report_flag & RCSP_REPORT_SMART_SEND_STATUS)
		{
			printf("send speex status !!!!!\n");
			Post_smart_command();
			rcsp_com_report_flag &= (~RCSP_REPORT_SMART_SEND_STATUS);
			break;
		}
