...
static u8 post_msg_flag = 0;///删除线程是否需要发送结束消息标志
...
*/
/*----------------------------------------------------------------------------*/
void play_sel_stop_decoder(_MUSIC_API *mapi, _PLAY_SEL_API *psel, u8 del_task)
{
    ...
    if(psel->busy)
    {
        ...
        {
			printf("play sel send stop msg !!!!\n");
			if((del_task == 1) && (post_msg_flag == 1))
			{
				printf("play sel no need post msg\n");
			}
			else
			{
...
__run_loop_psel:
        ...
        case SYS_EVENT_BEGIN_DEC:
            ...
            {
                play_sel_stop_decoder(mapi->dop_api,psel, 0);
            }
            break;
        case MSG_MUSIC_PP:
        ...
        case SYS_EVENT_DEL_TASK:
            ...
            play_sel_stop_decoder(mapi->dop_api,psel, 1);
...
/* NOTICE_PLAYER_ERR notice_player_play_by_num() */
/* { */
...
NOTICE_PLAYER_ERR notice_player_play_by_path(void *father_name, char *path, tbool (msg_deal)(void *priv, int msg[]), void *priv)
{
	...
	printf("stop!!!!\n");
	play_sel_stop();
	tbool ret = tone_play_by_name(father_name, 1, path);
	if(ret == false)
	{
		return 	NOTICE_PLAYER_PLAY_ERR;
	}

	if(msg_deal == NULL)
	{
		return 	NOTICE_PLAYER_NO_ERR_WITHOUT_MSG_DEAL;
	}

    int msg[6];
	u8 exit = 0;
	NOTICE_PLAYER_ERR err = NOTICE_PLAYER_NO_ERR;
	while(1)
	{
		...
		switch(msg[0])
		{
			case SYS_EVENT_DEL_TASK:
				exit = 1;
				err = NOTICE_PLAYER_TASK_DEL_REQ_ERR;
				break;		
			case SYS_EVENT_PLAY_SEL_END:
				...
				break;
			default:
				if(msg_deal(priv, msg) == true)
				{
					exit = 1;	
					err = NOTICE_PLAYER_MSG_BREAK_ERR;
				}
				else
				{
					os_taskq_msg_clean(msg[0], 1);
