...
s32 app_usb_slave_bulk_tx(const u8 *buf,u32 len)
{
    printf("%s() %d\n",__func__,len);
	USB_SLAVE_DEV_IO_REG();
    struct usb_io_t io_t;
    io_t.buf = buf;
    io_t.len = len;
    io_t.ot = 100;
	return __this(dev_ioctrl)(&io_t, USB_SLAVE_BULK_TX);
}
s32 app_usb_slave_intr_tx(const u8 *buf,u32 len)
{
    printf("%s() %d\n",__func__,len);
	USB_SLAVE_DEV_IO_REG();
    struct usb_io_t io_t;
    ...
    io_t.len = len;
    io_t.ot = 100;
	return __this(dev_ioctrl)(&io_t, USB_SLAVE_INRT_TX);
}
s32 intr_rx_func(u8 *buf,u32 len)
{
    printf("%s() %d\n",__func__,len);
    printf_buf(buf,len);        
    //TODO:echo data
    app_usb_slave_intr_tx(buf,len);
    return len;
}
s32 bulk_rx_func(u8 *buf,u32 len)
{
    printf("%s() %d\n",__func__,len);
    printf_buf(buf,len);        
    //TODO:echo data
    app_usb_slave_bulk_tx(buf,len);
    return len;
}
s32 app_usb_slave_register_bulk_rx(usb_io_cb_func cb_func)
{
	USB_SLAVE_DEV_IO_REG();
	return __this(dev_ioctrl)(cb_func, USB_SLAVE_BULK_RX);
}
s32 app_usb_slave_register_intr_rx(usb_io_cb_func cb_func)
{
	USB_SLAVE_DEV_IO_REG();
	return __this(dev_ioctrl)(cb_func, USB_SLAVE_INRT_RX);
}
#endif
/*----------------------------------------------------------------------------*/
...
#endif
#if VENDOR_CLASS_ENABLE
    app_usb_slave_register_bulk_rx(bulk_rx_func);
    app_usb_slave_register_intr_rx(intr_rx_func);
#endif
	return res;
}
