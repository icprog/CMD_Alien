...
/* reverb_init_test(ENC_LINE_RIGHT_CHANNEL); */

			*mode_reverb = (void*)reverb_init(reverb_switchv, ENC_LINE_RIGHT_CHANNEL);
            /* *mode_reverb = (void*)reverb_init(reverb_switchv, ENC_LINE_LEFT_CHANNEL); */
            /* *mode_reverb = (void*)reverb_init(reverb_switchv, ENC_MIC_CHANNEL); */
            if(*mode_reverb)
			{
				...
				dac_AmuxGain_en(1);//aux模拟放大输出放大一倍
    ...
    else
    {
		...
		/* set_sys_vol(dac_ctl.sys_vol_l,dac_ctl.sys_vol_r,FADE_ON); */
		dac_AmuxGain_en(0);//aux模拟放大输出放大一倍
#if (PITCH_EN == 0)
		/* mic_2_LR(0, 0); */
#endif
