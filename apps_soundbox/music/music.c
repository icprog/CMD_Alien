...
#if MUSIC_LRC_DECRYPT_EN

static CIPHER *lrc_cipher = NULL;

bool lrc_cipher_seek(void *hdl,u8 type,u32 offsize)
{
	return fs_seek(hdl, type, offsize);	
}

u16 lrc_cipher_read(void *hdl, u8 *buf, u16 len)
{
	u32 faddr = fs_tell(hdl);	
	u16 r_len = fs_read(hdl, buf, len);	
	if((r_len != 0) && (r_len != (u16)-1))
	{
		cipher_analysis_buff(lrc_cipher, buf, faddr, len);	
		return r_len;
	}
	return 0;
}

static const LRC_FILE_IO lrc_cipher_file_io = 
{
	.seek = lrc_cipher_seek,//fs_seek,//		
	.read = lrc_cipher_read,//fs_read,//		
	/* .seek = fs_seek,//		 */
	/* .read = fs_read,//		 */
};
#endif//MUSIC_LRC_DECRYPT_EN



#if MUSIC_DECRYPT_EN

static CIPHER *dec_cipher = NULL;

static void dec_cipher_analysis_buff(void* buf, u32 faddr, u32 len)
{
	if(dec_cipher == NULL)
		return ;

	cipher_analysis_buff(dec_cipher, buf, faddr, len);	
}
...
int music_decoder_init(MUSIC_OP_API **mapi_out)
{
    ...
    do
    {
        ...
        mapi->dop_api->dec_api.type_enable = DEC_PHY_MP3 | DEC_PHY_WAV |  \
											 ...
											 DEC_PHY_APE | DEC_PHY_AMR | \
											 DEC_PHY_M4A | DEC_PHY_DTS;//使能响应的解码库
...
void before_decoder_callback(MUSIC_OP_API *m_op_api,u32 dev_let)
{
	...
	{
		tbool ret = false;
		LRC_FILE_IO *lrc_io = NULL;
#if LRC_LYRICS_EN

#if MUSIC_LRC_DECRYPT_EN
        if(0 == lrc_find(m_op_api, "LRCLMP"))
        {
			ret = cipher_check_file_type(lrc_cipher, m_op_api->fop_api->cur_lgdev_info->lg_hdl->lrc_fhdl);
			lrc_io = (LRC_FILE_IO *)&lrc_cipher_file_io;
#else
		if(0 == lrc_find(m_op_api, "LRC"))
		{
#endif//MUSIC_LRC_DECRYPT_EN

			if(FALSE == lrc_analysis(m_op_api->fop_api->cur_lgdev_info->lg_hdl->lrc_fhdl, lrc_io))
			{
				printf("analazy err \n");
			}
			else
...
#if MUSIC_DECRYPT_EN
	cipher_check_file_type(dec_cipher, m_op_api->dop_api->io->f_p);
...
*/
/*----------------------------------------------------------------------------*/
u32  music_play_by_path(MUSIC_OP_API *mapi, char *path, u32 file_num)
{
    u32 err;
	if(mapi == NULL)
...
#if MUSIC_DECRYPT_EN
	dec_cipher = cipher_obj_creat(0x12345678, "SMP");
	reg_dec_input_cb(dec_cipher_analysis_buff);
	/* cipher_init(0x12345678); */
#endif // MUSIC_DECRYPT_EN


#if MUSIC_LRC_DECRYPT_EN
	lrc_cipher = cipher_obj_creat(0x12345678, "LMP");
#endif//MUSIC_LRC_DECRYPT_EN
...
#if MUSIC_DECRYPT_EN
			cipher_obj_destroy(&dec_cipher);
			/* cipher_close(); */
#endif
#if MUSIC_LRC_DECRYPT_EN
			cipher_obj_destroy(&lrc_cipher);
#endif//MUSIC_LRC_DECRYPT_EN
