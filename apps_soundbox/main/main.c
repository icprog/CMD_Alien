...
void AI_toy_dev_online_ok(void);
static void TaskMain(void *p)
{
...
#if DAC_AUTO_MUTE_EN
	digit_auto_mute_set(AUTO_MUTE_CFG, 4, 600, 240);
#endif
	...
	os_time_dly(150);
	while(g_powerdown_flag)
	{
		/* printf("power is too low, no power on\n"); */
        clear_wdt();
		os_time_dly(100);
	}
	...
	while(1)
    {
        ...
        case SYS_EVENT_LGDEV_ONLINE:
			...
			if(msg[1] == (int)sd0)
			{
				printf("is sd0 online\n");	
				AI_toy_dev_online((DEV_HANDLE)msg[1]);
				/* AI_toy_creat_dir(); */
				/* AI_toy_dev_online_ok();	 */
			}
...
//                    task_switch(RTC_TASK_NAME, 0,SWITCH_SPEC_TASK);
        ...
        case SYS_EVENT_LGDEV_OFFLINE:
			...
			if(msg[1] == (int)sd0)
			{
				printf("is sd0 offline\n");	
				AI_toy_dev_offline((DEV_HANDLE)msg[1]);
			}
...
//            Post_msg_to_rcsp_Task(MSG_RSCP_CHANGE_WORDMODE_ACTION,0,0);
		...
		case MSG_LAST_WORKMOD:
			if(compare_last_task_name(IDLE_TASK_NAME))
			{
				printf("last task is idle, no change\n");
				break;	
			}
			puts("MSG_LAST_MODE\n");
...
//            Post_msg_to_rcsp_Task(MSG_RSCP_CHANGE_WORDMODE_ACTION,0,0);
        ...
        case MSG_UPDATA:
			...
			{
				music_enter_updata();
			}else if(msg[1]==BT_UPDATE_AUTH_CODE){
				puts("MSG_UPDATA BT_UPDATE_AUTH_CODE\n");
                clear_wdt();
	            R_LED_ON();
	            B_LED_ON();
