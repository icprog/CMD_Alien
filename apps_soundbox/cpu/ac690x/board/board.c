...
void set_g_sys_cfg(FLASH_SYS_CFG *sys_cfg,u32 cfg_addr)
{
    ...
    for(i=0;i<app_use_flash_cfg.cfg_zone_size;i+=16)
    {
        ...
        else if(memcmp((const char *)cfg_data,"AUIF",4)==0)
		{
			tmp=(u32 *)(cfg_data+4);
			app_use_flash_cfg.auif_addr=*tmp;
			tmp=(u32 *)(cfg_data+8);
			app_use_flash_cfg.auif_len=*tmp;
			printf("au_addr =%08x \n",app_use_flash_cfg.auif_addr);
			printf("au_len =%08x \n",app_use_flash_cfg.auif_len);
