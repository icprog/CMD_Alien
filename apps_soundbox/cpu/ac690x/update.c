...
#define DEVICE_UPDATE_FILE_ERR BIT(29)
...
{   
    ...
    if((update_flag&DEVICE_FIRST_START)||(update_flag&DEVICE_UPDATE_KEY_ERR)||(update_flag&DEVICE_UPDATE_FILE_ERR))
    {
...
{
	if((update_flag&DEVICE_UPDATE_KEY_ERR) || (update_flag&DEVICE_UPDATE_FILE_ERR))
	{
...
int update_poweron_check(void)
{
	...
	if(update_flag&DEVICE_UPDATE_FILE_ERR)
	{
        puts("\n----------------update file err !!---------------- \n");
		return -2;
	}
...
void bt_updata_auth_code_deal()
{

	puts("----bt_updata_auth_code_deal\n");
	os_taskq_post_msg("MainTask",3,MSG_UPDATA,BT_UPDATE_AUTH_CODE, NULL);
