...
void ladc_isr_callback(void *ladc_buf,u32 buf_flag,u32 buf_len)
{
	...
	digital_volume_ctl(ladc_mic,DAC_SAMPLE_POINT,1);

	sync_adc_sr(ladc_mic);

#if (SMART_SOUNDBOX_EN)
	speex_encode_ladc_isr_callback(ladc_buf, buf_flag, buf_len);
...
#else
	ladc_input_fun(p_ladc, ladc_r, buf_flag, DAC_DUAL_BUF_LEN);
#endif
